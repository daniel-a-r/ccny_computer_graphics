...
HW3b::resizeGL(int w, int h)
{
	...
	// Set near, far planes and field of view
    const float zNear = 0.01, zFar = 100.0, fov = 45; //60 degrees;

	// compute persepective projection
	m_projection.setToIdentity();
	m_projection.perspective(fov, ar, zNear, zFar);
}
	...
	for(int i=0; i< m_grid; ++i) {
	   for(int j=0; j< m_grid; ++j) {
		...
		switch(m_geometryMode) {
			...
			case DIAGONALWALL:
				// PUT YOUR CODE HERE
				vec.setZ((j==(m_grid-1)-i) ? 0.5f : 0.0f);
				break;
			case SIDEWALL:
				// PUT YOUR CODE HERE
				vec.setZ((i==0 ? 0.5f : 0.0f));
				break;
			case DIAGONALBLOCK:
				// PUT YOUR CODE HERE
				vec.setZ((j>=(m_grid-1)-i) ? 0.5f : 0.0f);
